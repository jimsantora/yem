...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        "--use_txt_as_prompts",
        action="store_true",
        help="Use text files as prompts for corresponding images",
    )
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        center_crop=False,
        random_flip=False,
        use_txt_as_prompts=False  # New parameter
    ):
        self.size = size
        ...
        instance_images = []
        image_paths = []  # Keep track of paths for prompt matching
        for ext in image_extensions:
            found_paths = list(self.instance_data_root.glob(ext))
            image_paths.extend(found_paths)
            instance_images.extend([Image.open(path) for path in found_paths])

        # Handle prompts
        self.custom_instance_prompts = None
        if use_txt_as_prompts:
            prompts = []
            for img_path in image_paths:
                ...
                if txt_path.exists():
                    ...
                    prompts.append(instance_prompt)  # Fallback to default prompt
            # Extend prompts same as images
            self.custom_instance_prompts = []
            for prompt in prompts:
                self.custom_instance_prompts.extend(itertools.repeat(prompt, repeats))
...
def main(args):
    ...
    if args.with_prior_preservation:
        ...
        if cur_class_images < args.num_class_images:
            ...
            # Suggested here: https://huggingface.co/docs/diffusers/optimization/mps
